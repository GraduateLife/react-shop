import { readDocumentsAsArr, writeOne } from "../firebase/write-and-read";

import { Category } from "../models/category.type";

export const CateEntity: Category[] = [
  {
    CateName: "hat",
    CateImageUrl:
...
export const writeCategories = async () => {
  CateEntity.forEach(async (c) => {
    await writeOne<Category>("categories", c.CateName, c);
  });
  return "ok";
};
export const readCategories = async () =>
  await readDocumentsAsArr<Category>("categories");
